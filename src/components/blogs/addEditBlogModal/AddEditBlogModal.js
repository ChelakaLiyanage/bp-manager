    ...
    return (
        <Modal show={show} onHide={handleClose}>
            <Modal.Header closeButton>
                <Modal.Title>{isEdit ? BloggerHelper.ADD_EDIT_BLOG_MODAL.EDIT_BLOG_MODAL_TITLE : BloggerHelper.ADD_EDIT_BLOG_MODAL.ADD_BLOG_MODAL_TITLE}</Modal.Title>
            </Modal.Header>
            <Formik
                enableReinitialize
                initialValues={data ? { title: data.title, author: data.author, description: data.description } : { title: "", author: "", description: "" }}
                    ...
                    <Form onSubmit={handleSubmit}>
                        <Modal.Body>
                            ...
                            <Form.Group className="mb-3" controlId="formBasicEmail">
                                ...
                                <Form.Control type="text" placeholder="Enter author" name="author" value={values.author} onChange={handleChange} />
                                {errors.author && <Form.Text className="text-danger">{errors.author}</Form.Text>}
                            </Form.Group>
                            <Form.Group className="mb-3" controlId="formBasicEmail">
                                <Form.Label>Description</Form.Label>
                                <Form.Control as="textarea" placeholder="Enter description" name="description" value={values.description} onChange={handleChange} />
                        ...
                        <Modal.Footer>
                            ...
                            <Button variant="primary" type="submit" disabled={!isValid || !dirty}>
                                {isEdit ? BloggerHelper.ADD_EDIT_BLOG_MODAL.SUBMIT.UPDATE : BloggerHelper.ADD_EDIT_BLOG_MODAL.SUBMIT.ADD}
                            </Button>
                        </Modal.Footer>
                    </Form>
                )}
